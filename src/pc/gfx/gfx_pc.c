...
static bool gfx_texture_cache_lookup(int tile, struct TextureHashmapNode **n, const uint8_t *orig_addr, uint32_t fmt, uint32_t siz) {
    ...
    *node = &gfx_texture_cache.pool[gfx_texture_cache.pool_pos++];
    ...
    (*node)->siz = siz;
    *n = *node;
    // printf("Newly loaded texture ------ texture_addr = %d, hash = %d, siz = %d, fmt = %d\n", orig_addr, hash, siz, fmt);
    return false;
}
...
static void import_texture_ci4(int tile) {
    ...
    if (mode == G_TT_IA16) {
        for (uint32_t i = 0; i < rdp.loaded_texture[tile].size_bytes * 8; i++) {
            uint8_t byte = rdp.loaded_texture[tile].addr[i / 2];
            uint8_t idx = (byte >> (4 - (i % 2) * 4)) & 0xf;
            // printf("byte %0X\n", byte);
            // printf("idx  %d\n", idx);
            uint8_t intensity = rdp.palette[idx * 2];
            uint8_t a = rdp.palette[idx * 2 + 1];
            // printf("i %10X\n", intensity);
            // printf("a %10X\n", a);
            // fflush(stdout);
    ...
    if (mode == G_TT_IA16) {
        printf("width  %d\n", width);
        printf("height %d\n", height);
        printf("rdp.texture_tile.line_size_bytes  %d\n", rdp.texture_tile.line_size_bytes);
        printf("rdp.loaded_texture[tile].size_bytes  %d\n\n", rdp.loaded_texture[tile].size_bytes);
        fflush(stdout);
    }
    
    gfx_rapi->upload_texture(rgba32_buf, width * 2, height * 2);
}
