...
u32 interact_fruit(struct MarioState *, u32, struct Object *);
...
u32 interact_water_ring(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    ...
    if (m->action == ACT_DOLPHIN_ATTACK) {
        m->actionTimer = 0;
        m->actionState = 1;
    }
...
u32 interact_fruit(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    u32 fruitIndex;
    u32 noExit = (o->oInteractionSubtype & INT_SUBTYPE_NO_EXIT) != 0;
    ...
    if (m->health >= 0x100) {
        ...
        fruitIndex = (o->oBehParams >> 24);
        save_file_collect_fruit(fruitIndex);
