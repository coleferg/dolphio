 ...
 */
#define CAM_YAW_SPEED 0x800 
s16 sYawSpeed = CAM_YAW_SPEED;
...
void mode_8_directions_camera(struct Camera *c) {
    ...
    if (gPlayer1Controller->buttonPressed & R_CBUTTONS) {
        if (gPlayer1Controller->buttonDown & L_TRIG) {
            s8DirModeYawOffset = gMarioObject->header.gfx.angle[1] - DEGREES(90);
        } else {
        ...
        play_sound_cbutton_side();
    } else if (gPlayer1Controller->buttonPressed & D_CBUTTONS && gPlayer1Controller->buttonDown & L_TRIG) {
        s8DirModeYawOffset = gMarioObject->header.gfx.angle[1] + DEGREES(180);
        play_sound_cbutton_side();
    } else if (gPlayer1Controller->buttonPressed & U_CBUTTONS && gPlayer1Controller->buttonDown & L_TRIG) {
        s8DirModeYawOffset = gMarioObject->header.gfx.angle[1];
        play_sound_cbutton_side();
    }
 ...
 * Used when Mario is in a cannon.
 */
s32 update_in_jelly(UNUSED struct Camera *c, Vec3f focus, Vec3f pos) {
    s16 camYaw = s8DirModeBaseYaw + s8DirModeYawOffset;
    s16 pitch = look_down_slopes(camYaw);
    f32 posY;
    f32 focusY;
 ...
 * sCannonYOffset is used to make the camera rotate down when Mario has just entered the cannon
 */
void mode_jelly_camera(struct Camera *c) {
    vec3f_set_dist_and_angle(
        gMarioState->pos,
        c->pos,
        1800.0f,
        DEGREES(10),
        gMarioState->faceAngle[1] + DEGREES(180)
    );
    focus_on_mario(
        c->focus,
        c->pos,
    ...
    // lakitu_zoom(400.f, 0x900);
    // c->nextYaw = update_in_jelly(c, c->focus, pos);
    // c->pos[0] = pos[0];
    // c->pos[1] = pos[1];
...
// #define DEFAULT_POS_HSPEED 0.3f
// #define DEFAULT_POS_VSPEED 0.3f
// #define DEFAULT_FOC_HSPEED 0.8f
// #define DEFAULT_FOC_VSPEED 0.3f
...
void update_lakitu(struct Camera *c) {
    ...
    if (gCameraMovementFlags & CAM_MOVE_PAUSE_SCREEN) {
        ...
        set_or_approach_f32_asymptotic(&gLakituState.focHSpeed, DEFAULT_FOC_HSPEED, 0.05f);
        set_or_approach_f32_asymptotic(&gLakituState.focVSpeed, DEFAULT_FOC_VSPEED, 0.05f);
        set_or_approach_f32_asymptotic(&gLakituState.posHSpeed, DEFAULT_POS_HSPEED, 0.05f);
        set_or_approach_f32_asymptotic(&gLakituState.posVSpeed, DEFAULT_POS_VSPEED, 0.05f);
...
void init_camera(struct Camera *c) {
    ...
    gLakituState.mode = c->mode;
    gLakituState.defMode = c->defMode;
    gLakituState.posHSpeed = DEFAULT_POS_HSPEED;
    gLakituState.posVSpeed = DEFAULT_POS_VSPEED;
    gLakituState.focHSpeed = DEFAULT_FOC_HSPEED;
    gLakituState.focVSpeed = DEFAULT_FOC_VSPEED;
...
//             break;
    ...
    if (c->mode == CAMERA_MODE_8_DIRECTIONS) {
        gCameraMovementFlags |= CAM_MOVE_ZOOMED_OUT;
        // s8DirModeYawOffset = sMarioCamState->faceAngle[1];
        s8DirModeYawOffset = gMarioObject->header.gfx.angle[1] + DEGREES(180);
        // focus_on_mario(gMarioState->pos, marioOffset, 0.0f, 0.0f, 800.0f,  gMarioState->)
    }
    switch (gCurrLevelArea) {
...
s32 radial_camera_input(struct Camera *c, UNUSED f32 unused) {
    ...
    // Zoom in / enter C-Up
    if (gPlayer1Controller->buttonPressed & U_CBUTTONS && !(gPlayer1Controller->buttonDown & L_TRIG)) {
        if (gCameraMovementFlags & CAM_MOVE_ZOOMED_OUT) {
            gCameraMovementFlags &= ~CAM_MOVE_ZOOMED_OUT;
            play_sound_cbutton_up();
        } else {
    ...
    // Zoom out
    if (gPlayer1Controller->buttonPressed & D_CBUTTONS && !(gPlayer1Controller->buttonDown & L_TRIG)) {
        if (gCameraMovementFlags & CAM_MOVE_ZOOMED_OUT) {
...
void follow_mario_around_object(struct Camera *c) {
    ...
    if (gCutsceneFocus != NULL) {
        ...
        angleFromMarioToObject = obj_angle_to_object(gCutsceneFocus, gMarioObject);
 ...
 * Store camera info for the star spawn cutscene
 */
void follow_mario_around_object_shake(struct Camera *c) {
    Vec3f objectPos;
    s32 angleFromMarioToObject;
    f32 unusedDist;
    s16 unusedPitch;
 ...
 * Focus on the top of the star.
 */
BAD_RETURN(s32) cutscene_mario_focus_object(struct Camera *c) {
    UNUSED f32 hMul;
    Vec3f starPos;
    UNUSED f32 vMul;
 ...
 * Focus on the top of the star.
 */
BAD_RETURN(s32) cutscene_mario_focus_object_shake(struct Camera *c) {
    UNUSED f32 hMul;
    Vec3f starPos;
    UNUSED f32 vMul;
    ...
    if (gCutsceneFocus != NULL) {
        object_pos_to_vec3f(starPos, gCutsceneFocus);
        starPos[1] += gCutsceneFocus->hitboxHeight;
        approach_vec3f_asymptotic(c->focus, starPos, 0.025f, 0.03f, 0.025f);
        approach_vec3f_asymptotic(c->pos, starPos, 0.002f, 0.025f, 0.002f);
        set_camera_pitch_shake(70, 1, 0x6000);
    }
}
 ...
 * Plays when a star spawns (ie from a box).
 */
BAD_RETURN(s32) cutscene_pov_to_object(struct Camera *c) {
    cutscene_event(store_info_mario_and_object, c, 0, 0);
    cutscene_event(cutscene_mario_focus_object, c, 0, 300);
    cutscene_event(follow_mario_around_object, c, 300, -1);
    sStatusFlags |= CAM_FLAG_SMOOTH_MOVEMENT;
 ...
 * Plays when a star spawns (ie from a box).
 */
BAD_RETURN(s32) cutscene_pov_to_object_fast(struct Camera *c) {
    cutscene_event(store_info_mario_and_object, c, 0, 0);
    cutscene_event(cutscene_mario_focus_object_shake, c, 0, 300);
    cutscene_event(follow_mario_around_object_shake, c, 300, -1);
    sStatusFlags |= CAM_FLAG_SMOOTH_MOVEMENT;
 ...
 * Cutscene that plays when a star spawns from ie a box or after a boss fight.
 */
struct Cutscene sCutscenePOVtoObjectFast[] = {
    { cutscene_pov_to_object_fast, CUTSCENE_LOOP },
    { cutscene_star_spawn_back, 15 },
    { cutscene_star_spawn_end, 0 }
};
...
u8 sZoomOutAreaMasks[] = {
	...
	ZOOMOUT_AREA_MASK(0, 0, 0, 0, 0, 0, 0, 0), // CASTLE_GROUNDS | BITDW
	ZOOMOUT_AREA_MASK(0, 0, 0, 0, 0, 0, 0, 0), // VCUTM          | BITFS
	ZOOMOUT_AREA_MASK(0, 0, 0, 0, 1, 0, 0, 0), // SA             | BITS
	ZOOMOUT_AREA_MASK(1, 0, 0, 0, 0, 0, 0, 0), // LLL            | DDD
	ZOOMOUT_AREA_MASK(1, 0, 0, 0, 0, 0, 0, 0), // WF             | ENDING
	ZOOMOUT_AREA_MASK(0, 0, 0, 0, 0, 0, 0, 0), // COURTYARD      | PSS
	ZOOMOUT_AREA_MASK(0, 0, 0, 0, 1, 0, 0, 0), // COTMC          | TOTWC
	ZOOMOUT_AREA_MASK(1, 1, 0, 0, 1, 0, 0, 0), // BOWSER_1       | WMOTR
...
#define CUTSCENE(id, cutscene)                                                                            \
    ...
    switch (c->cutscene) {
        ...
        CUTSCENE(CUTSCENE_POV_TO_OBJECT_FAST, sCutscenePOVtoObjectFast)
