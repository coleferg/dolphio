...
void show_crash_screen_and_hang(void) {
    ...
    errno_crash = (cause >> 2) & 0x1F;
#ifndef TARGET_N64
    fb_fill(10, 10, 300, 220);
    while (1) // hang forever
    {
        UNUSED volatile int t = 0; // keep me happy
    }
#endif
    ...
    if (nAssertStopProgram == 0) {
        ...
        fb_print_int_hex(80, 30, errno_crash, 8);
        fb_print_str(107, 30, szErrCodes[errno_crash]);

        if (errno_crash >= 2 && errno_crash <= 5) {
            /*
