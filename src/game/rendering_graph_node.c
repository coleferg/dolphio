...
static void geo_append_display_list(void *displayList, s16 layer) {
    ...
    curObject = (struct Object *) gCurGraphNodeObject;

    displayLayer = layer == LAYER_CONTROLLED
        ? (curObject->oOpacity == 255 ? LAYER_ALPHA : LAYER_TRANSPARENT)
        : layer;
    ...
    if (gCurGraphNodeMasterList != 0) {
        ...
        listNode->next = 0;
        if (gCurGraphNodeMasterList->listHeads[displayLayer] == 0) {
            gCurGraphNodeMasterList->listHeads[displayLayer] = listNode;
        } else {
            gCurGraphNodeMasterList->listTails[displayLayer]->next = listNode;
        }
        gCurGraphNodeMasterList->listTails[displayLayer] = listNode;
    }
}
